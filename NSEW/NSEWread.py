...
def interpolate_and_average(dfs):
    ...
    if not dfs:
        ...
        return None, None  # Indicate no data was available
    ...
    if np.isnan(mean_values).all():
        logging.warning("All averaged values are NaN.")
        return common_bins, None  # Return common_bins but indicate that mean values are invalid
